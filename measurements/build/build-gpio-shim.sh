#!/bin/bash
# build-gpio-shim.sh — build Unikraft image with BCM2835 GPIO FreeBSD shim
# Run this script from inside the Docker build container.
# The repo is expected to be mounted at /workspace.

set -e

REPO=/workspace        # host repo mounted in container
...
# Overlay the local (uncommitted) shim files from the mounted repo.
# Once these are committed and pushed, this block can be removed.
...
# Copy headers that the build system expects in the Unikraft include tree
...
# Public GPIO API header (applications use <uk/gpio.h>)
...
# Download the unmodified FreeBSD GPIO driver (no edits needed)
mkdir -p ${RASPI_PLAT}/drivers/freebsd
if [ ! -f ${RASPI_PLAT}/drivers/freebsd/bcm2835_gpio.c ]; then
    curl -o ${RASPI_PLAT}/drivers/freebsd/bcm2835_gpio.c \
        "https://cgit.freebsd.org/src/plain/sys/arm/broadcom/bcm2835/bcm2835_gpio.c"
fi
...
cp ${SCRIPT_DIR}/gpio-shim-config .config
cp ${REPO}/measurements/gpio-shim/main.c ./main.c
